...
protected:
  ...
  double nom{};
public:
  double name() const{return nom;}
  void called(double no){nom=no;}
  void qui() const{cout <<" Primitive : "<<nom<<endl;}
  explicit Primitive(Point);
  Primitive() {}
...
public:
  Polygone(){}
  Polygone( Polygone&);
  explicit Polygone(Polygone*);
  explicit Polygone(Liste<Point>&Lpt,double libel=0) {init(Lpt,libel);}
  void init(Liste<Point>& ,double);
  Polygone (char*, double,
	    reel* mini=nullptr,reel* maxi= nullptr,
	    bool valid=true);
  ...
  Polygone (float(*)[3],double,reel* mini= nullptr,reel* maxi= nullptr);
 ~Polygone() override;
  void free();
  //	Polygone& operator = (Polygone&);
  Point& operator [] (int) override;
  // Fonctions d'acces aux membres prives de la classe
  int nb_sommet() override { return nb_sommets; }
  Point& sommets(int i) override { return sommet[i]; }
  Vecteur normal() override { return normale; }
  reel cst_equ() override { return cst_equ_plan; }
  void cst_equ(reel cst) override { cst_equ_plan=cst; }
  // Fonctions qui permet de savoir si un polygone est contenu dans une boite ou non
  int tout_point_inf(reel&, const int&) override;
  int tout_point_sup(reel&, const int&) override;
  int nb_in(reel&,reel&, const int&) override;
  void calc_cst_eq() override {
    calcul_normale_cst_equ(sommet[0],sommet [1],sommet[2]);
  }
  void calcul_normale_cst_equ(Point&, Point&, Point&);
  void show(const char* msg,ostream & out) override;
  // { out <<"Polygone[show] "<<msg<<endl;}
  double intersect(Param_Inter& parag,Point* I) override;
  double surface () override;
  Point centre() override {return isob;}
  Vecteur azi() override;
  double distance2_point(Point &) override; // carre de la distance du point P au polygone (convexe)
 bool appart_sphere(Point & O, double R2) override;
protected:
...
//    double intersect(Param_Inter& parag,Point *I); 
        void show(const char* msg,ostream &out) override { out << msg<<"-Triangle :"; qui();
