...
ITER *ip;
{
   ...
   for (ip->steps = 0; ip->steps < ip->limit; ) {
	 ...
	 fprintf(stderr,"<!> meschach<iter_mgcr> done=true (1) <- nres=%g\n",nres);/*MC97 sparc*/
	 done = TRUE; 
	 break;
      }
	 ...
	 if (i < ip->k - 1) {
	    ...
	    if ( nres <= MACHEPS*ip->init_res) { 
	       /* s_{i+1} == 0 */
	       i--;
	         fprintf(stderr,"<!> meschach<iter_mgcr> done=true (2)<-  nres=%g < %g* %g\n",nres,(double)MACHEPS,ip->init_res );/*MC97 sparc*/
		 done = TRUE;
	       break;
	    }
	    ...
	    if (sqrt(fabs(nres)) <= MACHEPS*ip->init_res)  {
	       /* s_k is zero */
	      i--;
	      fprintf(stderr,"<!> meschach<iter_mgcr> done=true (3) <- nres=%g\n",nres);/*MC97 sparc*/
	      done = TRUE;
	       break;
	    }
	    if (nres < 0.0) { /* do restart */
	...
	/* if (fabs((double)alpha->ve[i]) > dd)  {     
	 ...
	 if ( ip->stop_crit(ip,dd,VNULL,VNULL) ) {
	    /* stopping criterion is satisfied */
	     fprintf(stderr,"<!> meschach<iter_mgcr> done=true <-stop_crit\n"); /*MC97 sparc*/
	    done = TRUE;
	    break;
	 }
